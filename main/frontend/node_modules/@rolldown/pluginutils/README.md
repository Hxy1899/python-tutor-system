...
- `exactRegex(str: string, flags?: string): RegExp` ├ö├ç├Â Matches the exact string.
- `prefixRegex(str: string, flags?: string): RegExp` ├ö├ç├Â Matches values with the given prefix.
- `makeIdFiltersToMatchWithQuery(input: string | RegExp | (string | RegExp)[]): string | RegExp | (string | RegExp)[]` ├ö├ç├Â Adapts filters to match ids with query params.

### Composable Filters

- `and(...exprs)` / `or(...exprs)` / `not(expr)` ├ö├ç├Â Logical composition of filter expressions.
- `id(pattern, params?)` ├ö├ç├Â Filter by id (string or RegExp).
- `moduleType(type)` ├ö├ç├Â Filter by module type (e.g. 'js', 'tsx', or 'json').
- `code(pattern)` ├ö├ç├Â Filter by code content.
- `query(key, pattern)` ├ö├ç├Â Filter by query parameter.
- `include(expr)` / `exclude(expr)` ├ö├ç├Â Top-level include/exclude wrappers.
- `queries(obj)` ├ö├ç├Â Compose multiple query filters.
